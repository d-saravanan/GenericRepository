...
using GenericService.ServiceExceptions;
using System;
...
namespace GenericService.Services
{
    ...
    {
        private readonly GenericRepository.EntityFramework.IEntityRepository<TEntity, TId> _repository;

        /// <summary>
        /// Initializes a new instance of the <see cref="GenericService{TEntity, TId}"/> class.
        /// </summary>
        /// <param name="repository">The repository.</param>
        protected GenericService(GenericRepository.EntityFramework.IEntityRepository<TEntity, TId> repository)
        {
        ...
        /// <exception cref="EntityValidationException">Entity data validation failed</exception>
        /// <exception cref="EntityPostProcessorExceptions">Entity postprocessing failed</exception>
        public virtual async Task<IEnumerable<TEntity>> GetByIdsAsync(IEnumerable<TId> entityIds)
        {
            ...
            try {
                ...
                if (!ValidateACL(EntityOperations.Read, entityIds.ToArray())) {
                    UnPrivilegedAccess(entityIds); return Enumerable.Empty<TEntity>();
                }

                string message = string.Empty;

                entities = await _repository.GetByIdsAsync(entityIds.ToArray());
        ...
        /// <returns></returns>
        public abstract Task<PaginatedList<TEntity>> SearchAsync(EntitySearchCondition<TId> searchCondition);
        #endregion
